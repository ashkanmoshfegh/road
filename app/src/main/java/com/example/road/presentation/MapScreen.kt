...
class CurrentPositionOverlay(private val mapView: MapView) : Overlay() {
    ...
    private val haloPaint = Paint().apply { color = Color.WHITE; style = Paint.Style.FILL; isAntiAlias = true }
    private val dotPaint = Paint().apply { color = CURRENT_POS_COLOR; style = Paint.Style.FILL; isAntiAlias = true }
    private val arrowPaint = Paint().apply { color = Color.WHITE; style = Paint.Style.FILL; isAntiAlias = true }
...
fun MapScreen(
    ...
    // Only auto-recenter the map on currentPosition when this is true.
    // Previously the map called animateTo() on every position update
    // unconditionally, which meant the view kept snapping back while the
    // user was trying to pan around to place a start/destination point —
    // effectively "locking" scrolling. Now it only follows during
    // simulation (or whatever the caller decides warrants auto-follow).
